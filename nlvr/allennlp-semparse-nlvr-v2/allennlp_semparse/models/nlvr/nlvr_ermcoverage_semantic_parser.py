...
@Model.register("nlvr_erm_coverage_parser")
class NlvrERMCoverageSemanticParser(NlvrSemanticParser):
    """
    ...
    def __init__(
        ...
        super(NlvrERMCoverageSemanticParser, self).__init__(
            vocab=vocab,
        ...
        # Instantiating an empty NlvrLanguageFuncComposition just to get the number of terminals.
        self._terminal_productions = set(NlvrLanguageFuncComposition(set()).terminal_productions.values())
    ...
    def _get_state_cost(
        self, batch_worlds: List[List[NlvrLanguageFuncComposition]], state: CoverageState
    ) -> torch.Tensor:
        """
        ...
        try:
            # TODO(nitish): Fix execution errors
    ...
    def _get_state_info(
        self, state: CoverageState, batch_worlds: List[List[NlvrLanguageFuncComposition]]
    ) -> Dict[str, List]:
        """
