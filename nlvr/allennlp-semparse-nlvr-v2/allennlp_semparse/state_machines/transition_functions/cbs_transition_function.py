...
class CBSTransitionFunction(TransitionFunction[GrammarBasedState]):
    """
        ...
        for _, results in batch_action_probs.items():
            if allowed_actions and not max_actions:
                ...
                if self.cbs_mode:
                    ...
                    group_batch_size = max_actions // NUM_GROUP
                    new_batch_states = []
                    batch_states.sort(key=lambda x: x[0], reverse=True)
                    new_batch_states.extend(batch_states[:group_batch_size])
                    num_existing_groups = 1
                    del batch_states[:group_batch_size]

                    while batch_states:
                        augmented_scores = [[s[0], i] for i, s in enumerate(batch_states)]
                        done_indices = {}
                        for j, batch_state in enumerate(batch_states):
                            state_group_index = batch_state[1]
                            
                            state_action_history = state.action_history[state_group_index]

                            if state_group_index in done_indices.keys():
                                similarity_score = done_indices[state_group_index]
                            else:
                                similarity_score = 0

                                for k in range(num_existing_groups):

                                    existing_action_history = state.action_history[
                                        new_batch_states[k * NUM_GROUP][1]]

                                    sim_hist = self._similarity_between_histories(
                                        existing_action_history, state_action_history)

                                    similarity_score += sim_hist
                                
                                similarity_score /= num_existing_groups
                                done_indices[state_group_index] = similarity_score

                            augmented_scores[j][0] -= SIM_MULTIPLIER * similarity_score

                        augmented_scores.sort(key=lambda x: x[0], reverse=True)
                        new_idx = [x[1] for x in augmented_scores[:group_batch_size]]
                        for idx in sorted(new_idx, reverse=True):
                            new_batch_states.append(batch_states[idx])
                            del batch_states[idx]

                        num_existing_groups += 1

                    new_batch_states.sort(key=lambda x: x[0], reverse=True)
    ...
    def _similarity_between_histories(self, existing_action_history, state_action_history):
        sim_score = 0
        for s in range(len(existing_action_history)):
            if state_action_history[s] == existing_action_history[s]:
                sim_score += 1
            else:
                break
        return sim_score / len(state_action_history)