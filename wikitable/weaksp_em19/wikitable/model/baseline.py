...
from sempar.util import check_multi_col
...
class Programmer(nn.Module):
    ...
    def __init__(self,
        ...
        self.rnn = nn.LSTM(var_token_embed_size + pos_embed_size + token_indicator_size, 
                    rnn_hidden_size, 1, bidirectional=True, batch_first=True) # encoder
        ...
        self.sketch_prod_rnn = nn.LSTM(self.sketch_prod_embed_size, sketch_prod_rnn_hidden_size, 1, 
            bidirectional=True, batch_first=True) # AP encoder
    ...
    def forward(self, 
        ...
        for sketch_lf in sketch2program:
            ...
            if len(_paths) == 1:
                ...
                lf_log_likelihoods.append(seq_log_likeli)
                lfs.append(_paths[0])
                continue
            ...
            # lf_log_likelihoods += [seq_log_likeli + gs for gs in _gold_scores]
            lf_log_likelihoods += _gold_scores
            lfs += _gold_lfs

        if len(consistent_scores) > 0:
            return -1 * log_sum_exp(consistent_scores), lfs, lf_log_likelihoods
        else:
